...
1. [[concepts/15-prompt-chaining]] — 串行分解
2. [[concepts/16-routing]] — 分类路由
3. [[concepts/17-parallelization]] — 并行分片/投票
4. [[concepts/18-orchestrator-workers]] — 动态编排
5. [[concepts/19-evaluator-optimizer]] — 迭代优化
...
|---|---|---|
| [[concepts/15-prompt-chaining]] | Agent 读完 issue → 规划 → 改文件 → 跑测试，天然链式推进 | 链的长度和内容由 LLM 动态决定，不是代码写死的 |
| [[concepts/16-routing]] | Agent 根据子任务性质选择工具：文件编辑器 vs Shell vs 浏览器 | 分类器就是 LLM 自己，不是独立的分类步骤 |
| [[concepts/17-parallelization]] | Claude Code 可以并行发 Bash 调用；子 agent 并行工作 | 文章中的 Agent 图只画了串行循环，但实际实现支持并行 |
| [[concepts/18-orchestrator-workers]] | Agent 就是编排器，工具就是 worker | Workflow 版的 worker 是独立 LLM 实例；Agent 版的 worker 是工具调用 |
| [[concepts/19-evaluator-optimizer]] | Coding Agent 跑测试、看失败、迭代修改 | 评估由工具（测试框架）完成，不是独立评估器 LLM |

核心洞察：**五种 Workflow 模式是"冻结的设计决策"**——开发者在编码时决定何时串行、何时路由、何时并行。**Agent 则"融化"了这些模式**——LLM 在运行时自主做出同样的决策。这也印证了 [[concepts/08-bitter-lesson]]：预定义的 Workflow 逻辑是"人类知识的特定设计"，随着模型能力增长，可能消融为 Agent 的动态行为。

例外：[[concepts/17-parallelization]] 的 Voting 变体在 Agent 中较少见——Agent 通常不会对自己的输出投票。这是更偏 Workflow 层面的模式。

### "中心决策+分发"谱系

[[concepts/16-routing]] 和 [[concepts/18-orchestrator-workers]] 结构同源——都是"中心决策 → 分发到专门处理"，区别在于**选一个还是分多个**：
