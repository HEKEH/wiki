...
sources: [kubernetes/kubernetes基本概念.md, kubernetes/objects/namespace.md]
---
...
| 资源配额（[[entities/ResourceQuota]]） | ✅ 可 | 可限制每个 namespace 的 CPU/内存/对象数用量（配 LimitRange 设默认值） |
...
需额外定义 **[[entities/NetworkPolicy]]（网络策略）**——这才是 K8s 里做网络隔离的对象（如"`team-b` 只接受本 namespace 流量"）。但其是否生效**取决于 CNI 插件支持**（Calico/Cilium 支持；不支持的 CNI 即使写了也不生效），见 [[concepts/插件机制与可扩展性]]。

## 内置命名空间与操作要点

- `default`（默认）、`kube-system`（K8s 自带服务）、`kube-public`（公共信息，如 `cluster-info`）、`kube-node-lease`（节点心跳 Lease）。`default`/`kube-system` **不可删**。
- 状态 **Active / Terminating**；**删除 namespace 会级联删除其下所有资源**（危险操作）。名称 `[a-z0-9]([-a-z0-9]*[a-z0-9])?`、≤63 字符。
