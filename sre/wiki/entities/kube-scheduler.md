...
> **为何 `value` 与 `effect` 分成两个字段(不合并)**：二者正交——`key=value` 说"污点**关于什么**"(自由字符串、身份/原因)，`effect` 说"对不容忍者**怎么处置**"(固定枚举、调度器内建行为)。关键是**同一 key=value 常需配多个 effect**：如节点故障时 K8s 给同一 `node.kubernetes.io/not-ready` 先打 `NoSchedule`(不收新)、再打 `NoExecute`(驱逐旧)分级处置；且 toleration 能**按 effect 独立匹配**(如只容忍 NoSchedule、不容忍 NoExecute)。合并就无法"认原因、分动作"。另：`value` 可选、`effect` 必填。
>
