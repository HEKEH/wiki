...
- 对象模型：[[concepts/容器]] → [[entities/Pod]] → [[entities/Deployment]]（经 ReplicaSet 管理副本）；[[entities/Service]] 提供稳定入口；[[entities/Namespace]] 做逻辑隔离；[[entities/Node]] 承载运行。
- 组织与选择：[[concepts/Label与Selector]]。
- 存储与扩展：[[concepts/Volume存储]]、[[concepts/插件机制与可扩展性]]。
- 集群与运维：[[concepts/集群架构]]、[[concepts/扩缩容与滚动升级]]、[[concepts/健康检查]]、[[concepts/资源限制]]。
- 操作工具：[[entities/kubectl]]。

## 相关

- 源文档摘要：[[sources/kubernetes简介]]、[[sources/kubernetes基本概念]]、[[sources/kubernetes-101]]、[[sources/kubernetes-201]]、[[sources/kubernetes集群]]
