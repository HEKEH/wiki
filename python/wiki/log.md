...
## [2026-09-07] ingest | 复习题组 06 —— 三种并发模型与选型自测卷

**来源**：[[concurrency/concurrency-models]]（无新源文档，纯出题）。

**产出**：[[review/review-set-06]]，45 题（选择 18 / 判断 15 / 填空 12），
按主题页七节铺开：概念辨析、三模型量级对比表、决策树选型场景、四种写法的实测数字、
`concurrent.futures` 接口、asyncio + 线程池 + 进程池的混合模式、答题模板。
题型全部可机械判分，答案留空待作答。

**设计要点**：把主题页里最容易记反的四处做成陷阱题——`ex.map` 保序 vs `as_completed`
完成序（选择 17 多选 + 判断 10、11）、两个 Executor 的 `max_workers` 默认值
（选择 16 + 判断 7、8 + 填空 3、4）、`to_thread` 是线程池不是进程（判断 12）、
asyncio 对 CPU 密集是**负收益**而非零收益（选择 15 + 判断 9）。

**实测校准**（CPython 3.11.9，8 核）：`ThreadPoolExecutor()._max_workers` = 12
（`min(32, cpu_count+4)`）、`ProcessPoolExecutor()._max_workers` = 8；
向进程池提交 lambda 报 `_pickle.PicklingError: Can't pickle <function <lambda>...>`。

**待办**：作答后批改，把「参考答案 / 解析 / 面试落点」追加到该页末尾。

## [2026-09-13] query | 批改复习题组 06 选择题（14.5/18）

**作答**：[[review/review-set-06]] 的 18 道选择题，判断题与填空题留空待答。

**得分 14.5/18**，错 3 题 + 多选漏选 1 项：

- **#6**（选「错误」项）答 B 应为 **D** —— 审题失误，把正确陈述「进程数 ≈ CPU 核数」
  当成了错项；真正错的是「协程数量受 CPU 核数约束」（协程上限由内存决定，与核数无关）。
- **#15** 答 D 应为 **C** —— 正是出题时预告的坑：asyncio 对 CPU 密集是**负收益**不是零收益。
  脚本里确实「等同于串行」，但服务里一个不 await 的协程会卡死事件循环，**所有**请求超时。
- **#16** 答 B(8) 应为 **C(12)** —— 把 `ProcessPoolExecutor` 的默认值 `cpu_count()`
  套到了 `ThreadPoolExecutor` 的 `min(32, cpu_count+4)` 上。
- **#17**（多选）答 AB 漏 **C** —— 知道 `map` 保输入序，但没推到「异常惰性抛出、
  且一抛就中断整个迭代、拿不到剩余结果、无法反查是哪个 item」这层后果。

**新增解析**（追加为该页「批改记录（一）· 选择题」）：三种模型可扩展性上限的成因对照表
（线程卡 8 MB 栈 / 进程卡常驻内存与核数 / 协程只卡内存）、bcrypt 阻塞路由的
❌/✅ 对照写法、两个 Executor 默认值差异的**设计理由**（I/O worker 不占 CPU 所以可超配，
CPU worker 要占满核所以不超配）、`map` 惰性抛异常与 `{future: item}` 反查输入的实测对照。

**实测**（CPython 3.11.9 / 8 核）：`ex.map(fn,[1,2,3])` 在 `fn(2)` 抛错时先正常产出
`10`、迭代到第二项才抛 `ValueError: boom 2`；`ex.map(sleep_then,[3,1,2])` → `[3,1,2]`（输入序），
`as_completed` → `[1,2,3]`（完成序）。

**待办**：判断题 15 题 + 填空题 12 题作答后批改，追加「批改记录（二）」。

## [2026-09-13] query | FastAPI 里的 CPU 密集任务怎么不阻塞其它请求

**提问**：某个 service 需要长时间占 CPU，如何避免阻塞其它请求？要示例代码。

**产出**：新建 [[web/fastapi-cpu-bound]]。原有覆盖零散——[[web/fastapi-core]] §4 只说了
`def` 路由会进线程池、[[web/fastapi-auth]] 提了一句 bcrypt、[[concurrency/asyncio-patterns]]
列了三种 offload 写法——但没有一页把「为什么线程池不够」和「生产形态该怎么写」讲完整。

**实测**（CPython 3.11.9 / FastAPI 0.141.1 / uvicorn / 8 核，真实 HTTP 压测，
4 个并发 CPU 请求 + 另一条连接持续探 `/health`）：

| 方案 | 4 请求耗时 | `/health` 中位 | 最坏 |
|---|---|---|---|
| 空载基线 | — | 4.5ms | 5.9ms |
| `async def` 直接算 | 5.30s | **5262.9ms** | 5262.9ms |
| `await to_thread` | **8.87s** | 99.0ms | 686.3ms |
| `run_in_executor`(进程池 4) | **2.50s** | 10.1ms | 161.0ms |

两个反直觉发现，都写进了页里：

1. **`to_thread` 对纯 Python 计算总耗时反而最长（8.87s > 5.30s）**——4 个线程互抢 GIL，
   切换开销白给；它只是解除了「冻结」，中位延迟仍有 22 倍劣化。判据是
   「这段计算是不是纯 Python 字节码」，而非「像不像 CPU 任务」。
2. **`asyncio.wait_for` 超时杀不掉子进程**：`fut.cancelled()` 返回 `True` 是假象，
   `concurrent.futures.Future.cancel()` 对运行中的任务返回 `False`。
   实测超时返回后，1-worker 池上再提交的秒级任务等了 **14.61s**——子进程还在烧 CPU。
   页里给了四种真解法（任务内自检 deadline / `max_tasks_per_child` / `pebble` / Celery）。

**代码**：给出可直接抄的生产形态——`lifespan` 建池复用、`Semaphore` 背压（满了 503 而非
无限排队）、`wait_for` 超时、捕获 `BrokenProcessPool`、子进程 `initializer` 忽略 SIGINT。
实测 20 并发下 `{200: 8, 503: 12}`，超长任务返 504。
另记三个坑：`uvicorn workers × MAX_WORKERS ≈ 核数`、pickle 大对象不如传引用、
Windows/macOS spawn 需 `if __name__ == "__main__"` 守卫。

**顺带**：`BrokenProcessPool` 不在 `concurrent.futures` 顶层导出，
要从 `concurrent.futures.process` 导入（顶层只有基类 `BrokenExecutor`）。

**交叉引用**：[[web/fastapi-core]] 两个线程池的表后补了一句「线程池救不了纯 Python CPU」；
[[concurrency/concurrency-models]] 的「相关」加了回链；[[index]] 加目录行与导航路径。

## [2026-09-13] query | 批改复习题组 06 判断题（14/15）+ 修订一道缺陷题

**作答**：[[review/review-set-06]] 的 15 道判断题。填空题仍留白。

**得分 14/15**，唯一实错第 8 题；第 5 题判对，但记为**出题缺陷**。

- **#8**（`ProcessPoolExecutor` 默认 `max_workers` = `cpu_count()`）答 ❌ 应为 ✅。
  值得记录的是**错法的一致性**：选择题 #16 答 8、判断题 #8 答错，两次都指向
  「两个 Executor 的默认值记反了」这一个内部模型。回炉点收敛成一条。
- **#5** 原题干「接口完全一致，从线程切到进程只需改一个类名」是**前半真、后半假的复合命题**，
  用户答 ❌ 技术上更正确（复合命题有一个合取项为假即为假）。判其正确，
  并在题目下方加 ⚠️ 标注，说明重出时拆成两题：
  ① 接口签名一致（✅）② 只需改类名其余不动（❌）。
  缺陷成因：照抄主题页面向「接口统一」的修辞，没意识到会被当严格断言。
- **#9** 答 ❌ 正确——上一轮选择题 #15「负收益 vs 零收益」的错点已改过来。

**新增解析**（追加为「批改记录（二）· 判断题」）：

1. 两个 Executor 默认值的**理由表**（I/O worker 不占 CPU 所以可超配 / CPU worker 要占满核所以不超配），
   替代硬背数字。
2. 「换成进程池后还要动什么」六行表：pickle 约束、序列化成本、全局状态不再共享、
   spawn 的 `__main__` 守卫、logging 重配、traceback 残缺。
3. **第 6 题的重要补充**——判断题只说了「不需要 `threading.Lock`」，但没覆盖
   「**临界区跨越 `await` 仍需 `asyncio.Lock`**」。实测 check-then-act 的余额扣成 **-100**
   （单线程也能出竞态），并给出 `asyncio.Lock` 修正版输出。这是「单线程不用加锁」
   这个常见误解的正确边界，面试高频追问。

**待办**：填空题 12 题作答后批改，追加「批改记录（三）」。

## [2026-09-13] query | 批改复习题组 06 填空题（9.5/12），全卷 38/45；批改体例改为行内批注

**体例变更（本条最重要）**：应要求，批改不再另起「批改记录」区块，
**批注直接写在每题答案下方的引用块里**（`> ✅ **1/1**。…`）。本卷填空题起采用，
后续所有题组沿用。已在「批改记录（一）」开头加一句体例说明，说明选择/判断两节
是旧体例的遗留。旧的两个区块保留原样，未回改。

**填空题 9.5/12**。三处扣分**全是用词精度，没有一处机制理解错误**：

| # | 用户 | 应为 | 性质 |
|---|---|---|---|
| 2 | 用户 | **事件循环** | 答的是「在哪一态」而非「谁在调度」；调度者必须是主体 |
| 9 | 非lambda | **模块** | 只排除一种反例，没答出正面判据 |
| 12 | asyncio.run | **asyncio** | 把「进程入口函数」当成了「长期运行的主循环」 |

第 10 题空白（`Future` 五方法），0 分。

**亮点**：第 3 题 `min(32, cpu+4)` / `12` 全对——这个知识点在选择 #16、判断 #8
连错两次，到填空已完全纠正，原回炉清单第一条划掉。

**新发现的出题缺陷（第 10 题）**：题干写「**不阻塞**地拿异常对象用 `______`」是错的。
实测 `Future.exception()` **同样阻塞**：

```python
f = ex.submit(boom)          # boom 先 sleep(0.5) 再抛 ValueError
f.exception()                #=> 耗时 0.50s -> ValueError('kaboom')
f.exception(timeout=0)       #=> TimeoutError
```

两者真正的区别**不是阻塞与否，是抛出与返回**：`result()` 重抛，`exception()` 返回对象，
两者都等。真正非阻塞的只有 `done()` / `cancelled()` / `add_done_callback()`。
已在该题批注里给出完整五方法表 + JS Promise 对照，并写明重出时第 2 空应改为
「取回异常对象而不抛出」。这是本卷第二处出题缺陷（第一处是判断 #5 的复合命题）。

**全卷 38/45（84%）**：选择 14.5/18、判断 14/15、填空 9.5/12。
7 分扣分里**真正的机制理解错误只有 1 处**（选择 #15 负收益 vs 零收益），
且已在判断 #9 自行纠正。剩余回炉清单两条：`Future` 五方法、三个主体名词。

**批注里补的延伸知识**（读而不答，符合题型限定约定）：3.13 起
`ThreadPoolExecutor` 默认值改用 `os.process_cpu_count()`（尊重 CPU 亲和性）；
`cpu_count()` 在容器里读宿主核数的坑；`to_thread` 会传播 `contextvars`
而 `run_in_executor` 不会；`run_in_executor` 不支持 kwargs 需 `partial`；
pickle 存的是 `__module__` + `__qualname__` 导入路径（所以报错是
`attribute lookup ... failed`，而非「lambda 类型不支持」）；
`get_running_loop` vs `get_event_loop` 的弃用演进。

## [2026-09-13] lint | 把批改体例写进 CLAUDE.md，使其下次自动生效

**动因**：上一条只在题组 06 里实践了「批注写在每题答案下方」的新体例，
但没有落进约定文件——下次开新题组时不会自动沿用。

**改动**（`python/CLAUDE.md` 的 review 分类节）：

1. 新增 **「批改约定（2026-09-13 起）」** 八条：批注紧跟 `**答：**` 行、
   首行固定得分标记（`> ✅ **1/1**。` / `> ⚠️ **0.5/1**。` / `> ❌ **0/1**。`）、
   评分口径（多选漏选半分、填空按空均分）、节首分数行 + 节末小结表 + 全卷总结、
   **失分归因六分类**（机制理解错 / 纯记忆 / 审题 / 推论没走完 / 用词精度 / 空白）
   与跨题串联要求、出题缺陷要主动认并判读者对、延伸知识仍「读而不答」、
   批注输出必须实测、批改后同步 index 与 log。
2. **题型限定补上「填空题」**（第 4 类，「三类题」改为「四类题」）——
   题组 05、06 都已在用，属既成实践，此前未入约定。
   注明其定位是「考能否说出准确的主体名词」，专治「懂机制但用词含糊」。
3. **替换与现状不符的旧描述**：原写「单题结构固定为『题目/参考答案/解析/面试落点』四段」，
   但 05、06 实际是「成套自测卷 + 事后批改」。改为明确的**三段式工作流**：
   出卷（留空 `**答：**`）→ 作答 → 批改（批注入题下），并强调
   **出卷时不要预先写出答案与解析**，否则就成了阅读材料而非自测卷。

**归因设计的理由**（这是新约定里最有价值的一条）：题组 06 的 7 分扣分中，
真正的「机制理解错」只有 1 处，其余是审题、纯记忆、用词精度等——
如果只报分数，回炉清单会误列 5 条；按性质归因后只剩 2 条。
