...
第八条线索（来自 [[sources/why-programs-cant-access-memory]]）：并发不只争 CPU，也争**内存**——于是
需要 [[concepts/memory-protection]]。规则是"进程默认不能碰别的进程的地址空间"（否则支付缓冲区会被
读走/篡改、甚至代码被注入）。和第五条（context-switch）、第一条（mode-bit）一样，这又是"纯软件做不到、
**需硬件支持**"的情形：每次访存都校验，软件开销过大。方案是两个**特权寄存器**——基址+界限——在**地址
总线**上拦截每个地址，用比较器+AND 门判定 `base ≤ addr < base+limit`，再**门控读/写使能**信号；越界则
触发硬件信号、中断进程、唤起 OS（Unix 下即**段错误**）。OS 唯一职责是派发前设好寄存器，并在
[[concepts/context-switch]] 时更新边界。这是 [[concepts/memory-management-unit]] 的入门原型，更通用的
**分页**留作下一条线索。
...
- **分页(paging)** 如何取代连续的基址+界限方案，让系统使用比物理内存更多的内存、且无需连续分配？
  它与 [[concepts/page-fault]]（已建页）的按需调页/换页如何衔接？（源材料预告了后续一集。）
